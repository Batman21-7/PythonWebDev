...
class CafeForm(FlaskForm):
    ...
    wifi_rating = SelectField('WiFi Strength Rating', validators=[DataRequired()], choices=[
        '✘', '💪', '💪💪', '💪💪💪', '💪💪💪💪', '💪💪💪💪💪'])
    power_availability = SelectField('Power Socket Availability', validators=[DataRequired()], choices=[
        '✘', '🔌', '🔌🔌', '🔌🔌🔌', '🔌🔌🔌🔌', '🔌🔌🔌🔌🔌'])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        ...
        with open('cafe-data.csv', 'a', encoding='utf-8') as file:
            data = request.form
            new_cafe = (f"\n{data['cafe']},{data['location_url']},{data['opening_time']},{data['closing_time']},"
                        f"{data['coffee_rating']},{data['wifi_rating']},{data['power_availability']}")
            print(new_cafe)
            file.write(new_cafe)
